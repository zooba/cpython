...
class FileFinder:
    ...
    if sys._xoptions.get("preload"):
        def _fill_cache(self):
            """Fill the cache of potential modules and packages for this directory."""
            cached_contents = _imp._cache_path_names((self.path or _os.getcwd()) + "/__pycache__", True)
            contents = _imp._cache_path_names(self.path or _os.getcwd(), True)
            self._path_cache = contents
            if sys.platform.startswith(_CASE_INSENSITIVE_PLATFORMS):
