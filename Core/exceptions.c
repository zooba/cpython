                   ...
                   PyObject **nativeerror)
{
    ...
    /* TODO: Switch to proper parsing */
    nargs = PyTuple_GET_SIZE(args);

    if (nargs >= 2 && nargs <= 5) {
        if (!PyArg_UnpackTuple(args, "OSError", 2, 5,
                               myerrno, strerror,
                               filename, nativeerror, filename2))
            return -1;
        if (*nativeerror && PyLong_Check(*nativeerror)) {
            long errcode;
            size_t nativeerrcode;
            PyObject *newargs;

            nativeerrcode = PyLong_AsSize_t(*nativeerror);
            if (nativeerrcode == -1 && PyErr_Occurred())
                return -1;
            errcode = PyErr_NativeCodeToErrno(nativeerrcode);
             ...
             PyObject *nativeerror)
{
    ...
    if (filename && filename != Py_None) {
        ...
        if (nargs >= 2 && nargs <= 5) {
            /* filename, filename2, and nativeerror are removed from the args tuple
...
OSError_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    if (!oserror_use_init(type)) {
        if (oserror_init(self, &args, myerrno, strerror,
                         filename, filename2, nativeerror
            ))
            goto error;
    }
    else {
...
OSError_init(PyOSErrorObject *self, PyObject *args, PyObject *kwds)
{
    ...
    Py_INCREF(args);
    if (oserror_parse_args(&args, &myerrno, &strerror, &filename, &filename2, &nativeerror))
        goto error;

    if (oserror_init(self, &args, myerrno, strerror, filename, filename2, nativeerror))
        goto error;
...
#define OR_NONE(x) ((x)?(x):Py_None)
    /* If available, nativeerror has the priority over myerrno */
    if (self->nativeerror && self->filename) {
        if (self->filename2) {
            return PyUnicode_FromFormat("[Error %S] %S: %R -> %R",
                                        OR_NONE(self->nativeerror),
    ...
    if (self->nativeerror && self->strerror)
        return PyUnicode_FromFormat("[Error %S] %S",
                                    self->nativeerror ? self->nativeerror: Py_None,
...
static PyMemberDef OSError_members[] = {
    ...
    {"nativeerror", T_OBJECT, offsetof(PyOSErrorObject, nativeerror), 0,
        PyDoc_STR("Native error code")},
    {NULL}  /* Sentinel */
};
